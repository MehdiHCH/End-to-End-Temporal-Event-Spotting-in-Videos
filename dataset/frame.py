...
class FrameReader:
    ...
    def load_frames(self, video_meta, start, end, pad=False, stride=1, randomize=False):
        ...
        if isinstance(video_meta, str):
            video_name=video_meta
        elif isinstance(video_meta, dict):
            video_name=video_meta['video']
        #for frame_num in range(start, end, stride):
        for i in [1,2]:
            for frame_num in map(lambda event :event["frame"],video_meta['events']) :
                ...
                frame_path = os.path.join(
                    self._frame_dir, video_name,str(i),
                    FrameReader.IMG_NAME.format(frame_num))
                #print(frame_path,"-------- ha huwaa ")
                try:
                    img = self.read_frame(frame_path)
                    ### Débogage 
                    if img is None or img.numel() == 0:
                        print(f"Frame {frame_num} at {frame_path} is invalid or empty.")
                        continue
                    ##  Débogage      
                    ...
                    print(f"Frame {frame_num} not found at {frame_path}. Skipping.")
                    n_pad_end += 1
        #-------Débogage ----
        if len(ret) == 0:
            print(f"No valid frames loaded for video {video_name} from {start} to {end}.")
            raise ValueError(f"No frames were loaded. Ensure that files exist in {self._frame_dir}\{video_name}.")
        # print(f"Loaded {len(ret)} frames for video {video_name}. First frame shape: {ret[0].shape if len(ret) > 0 else 'N/A'}")

        #--------- Débogage -----------
        # In the multicrop case, the shape is (B, T, C, H, W)
        # print(int(len(ret[0].shape)))
        ret = torch.stack(ret, dim=int(len(ret[0].shape) == 4))
        if self._same_transform:
            ret = self._img_transform(ret)
        # Always pad start, but only pad end if requested
        max_frames = 1776  # Or use a variable to track the maximum dynamically
        current_frames = ret.shape[0]
        n_pad_end = max_frames - current_frames
...
class ActionSpotDataset(Dataset):
    ...
    def _get_one(self):
        ...
        return {'frame': frames[:self._clip_len], 'contains_event': int(np.sum(labels) > 0),
                'label': labels}
...
class ActionSpotVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for label in self._labels:
            if label["video"] == video_name:
                video_meta = label
        frames = self._frame_reader.load_frames(
            video_meta, start, start + self._clip_len * self._stride, pad=True,
            stride=self._stride)
