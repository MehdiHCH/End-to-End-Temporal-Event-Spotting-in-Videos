...
EPOCH_NUM_FRAMES = 1776

BASE_NUM_WORKERS = 4

BASE_NUM_VAL_EPOCHS = 10
...
def load_from_save(
    ...
    model.load(torch.load(os.path.join(
        args.save_dir, 'checkpoint_{:03d}.pt'.format(epoch)),weights_only=True))
    ...
    if args.resume:
        ...
        opt_data = torch.load(os.path.join(
            args.save_dir, 'optim_{:03d}.pt'.format(epoch)),weights_only=True)
...
def get_num_train_workers(args):
    ...
    print(f" le nombre de works est : {n}")
...
def main(args):
    ...
    # Write it to console
    store_config(sys.stdout, args, num_epochs, classes)
